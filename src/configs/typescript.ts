...
import { CONFIG_PREFIX, interopDefault, renameRules } from "../utils"
...
export async function typescript(
  ...
  function makeParser(typeAware: boolean, files: string[], ignores?: string[]): TypedFlatConfigItem {
    return {
      ...
      name: `${CONFIG_PREFIX}/typescript/${typeAware ? "type-aware-parser" : "parser"}`,
    }
  }

  return [
    {
      // install plugins without globs, so they can be configured separately
    ...
    ...isTypeAware
      ? [{
          ...
          name: `${CONFIG_PREFIX}/typescript/rules-type-aware`,
          rules: {
            ...typeAwareRules,
            ...overridesTypeAware,
          },
    ...
    ...erasableOnly
      ? [
          {
            name: `${CONFIG_PREFIX}/typescript/erasable-syntax-only`,
            plugins: {
